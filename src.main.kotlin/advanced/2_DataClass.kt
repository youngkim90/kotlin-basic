...
fun main() {
    ...
    println(person1 == person2) // data class는 비교 시 equals로 비교하기 때문에 내용이 동일하면 true

    val set = hashSetOf(person1)
    println(set.contains(person2)) // data class는 hashcode를 자동으로 생성하기 때문에 동일한 객체로 인식

    println(person1.toString()) // toString()이 포함되어 있음(생략가능)
    ...
    val (name, age) = person1 // 구조 분해 선언 방식
